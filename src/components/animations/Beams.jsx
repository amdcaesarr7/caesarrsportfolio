...
const hexToNormalizedRGB = hex => {
  ...
  return [parseInt(clean.substring(0, 2), 16) / 255, parseInt(clean.substring(2, 4), 16) / 255, parseInt(clean.substring(4, 6), 16) / 255];
};
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      if (j < heightSegments) { const a = vertexOffset, b = vertexOffset + 1, c = vertexOffset + 2, d = vertexOffset + 3; indices.set([a, b, c, c, b, d], indexOffset); indexOffset += 6; }
